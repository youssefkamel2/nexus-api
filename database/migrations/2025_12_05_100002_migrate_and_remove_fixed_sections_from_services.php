...
return new class extends Migration
{
    ...
    {
        // First, migrate existing data to service_sections table
        $services = DB::table('services')->get();
        
        foreach ($services as $service) {
            for ($i = 1; $i <= 3; $i++) {
                $content = $service->{"content$i"};
                $image = $service->{"image$i"};
    ...
    public function down(): void
    {
        ...
        // Migrate data back from service_sections (best effort - only first 3 sections)
        $services = DB::table('services')->get();
        
        foreach ($services as $service) {
            $sections = DB::table('service_sections')
